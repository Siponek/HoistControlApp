...
{
    char pipeName[sizeof(moveSize)+1];
    char staleMoveSize[80];
    strcpy(&staleMoveSize, moveSize);
    // strcat(&pipeName, "\0");
    pipeName[sizeof(pipeName)] = '\0';
    printf("To jest pipeName %s\n", pipeName);
    printf("To jest moveSize %s\n", moveSize);
    printf("To jest sizeof(pipeName) %lu\n", strlen(pipeName));
    printf("To jest sizeof(moveSize) %lu\n", strlen(moveSize));
...
int main(int argc, char const *argv[])
{
    ...
    while (1)
    {
        ...
        write(openPipe(pipeX),moveSize ,strlen(moveSize)+1);
